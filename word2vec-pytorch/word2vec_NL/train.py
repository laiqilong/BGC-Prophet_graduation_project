...
class Word2VecTrainer:
    def __init__(self, input_file, output_file, emb_dimension=200, batch_size=1024, window_size=5, iterations=50,
                 initial_lr=0.002, min_count=5):

        self.data = DataReader(input_file, min_count=min_count, window_size=window_size, negativeNum=5)
        # 读取文件，为耗时步骤
    ...
    def train(self):
        ...
        for iteration in range(self.iterations):
            ...
            for i, sample_batched in enumerate(tqdm(self.dataloader)):
                ...
                # 更改为每一百次记录一次loss
                if i > 0 and i % 200 == 0:
                    print(" Loss: " + str(loss.sum()/loss.numel()))
...
if __name__ == '__main__':
    w2v = Word2VecTrainer(input_file=r"ptb.train.txt", output_file="output_ptb")
    w2v.train()
