...
class DataReader:
    ...
    def __init__(self, inputFileName, min_count, window_size, negativeNum, mode='train'):
        ...
        # 函数的调用等同于初始化
            ...
            self.__loadCC()
            self.__loadNegatives()

    def __del__(self):
        if self.negativesLMDB:
            self.negativesLMDB.close()
            print("Negatives LMDB close!")
    ...
    def __loadNegatives(self):
        self.negativesLMDB = lmdb.open('./lmdb_negatives/', subdir=True, map_size=307374182400, readonly=True)
        self.txn = self.negativesLMDB.begin()
...
class Word2vecDataset(Dataset):
    def __init__(self, data):
        ...
        assert len(self.data.centers) == len(self.data.contexts)
    ...
    def __getitem__(self, idx):
        negative_pkl = self.data.txn.get(str(idx).encode('ascii'))
        negative = pickle.loads(negative_pkl)
        return (self.data.centers[idx], self.data.contexts[idx], negative)
