...
# 预测每个基因是否属于BGC
class TransformerEncoderLabelsTrainer:
    ...
    def train_label_step(self, epoch):
        ...
        torch.save(self.model, self.save_path + f'transformerEncoder_Model_labels_{epoch}.pt')
...
if __name__=='__main__':
    ...
    if args.load_label_model:
        ...
        model = transformerEncoderLabelsNet(d_model=embedding_dim, nhead=args.nhead, num_encoder_layers=args.num_encoder_layers, max_len=args.max_len, 
                                    dim_feedforward=embedding_dim*4, dropout=args.dropout, labels_num=data.labels_num,
                                    batch_first=True, norm_first=True, concat_type=args.concat_type)
    if args.two_gpu:
    ...
    trainer = TransformerEncoderLabelsTrainer(args=args, writer=writer,data=data, model=model, Labelloss=Labelloss)
    trainer.train()