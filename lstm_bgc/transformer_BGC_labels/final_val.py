...
class final_validation:
    ...
    def table(self, x_true, x_pred, label):
        ...
        fn = cm_percent[1][0]
        tp = cm_percent[1][1]
        print('TP:\t{:.3f}%\tFN:\t{:.3f}%\nFP:\t{:.3f}%\tTN:\t{:.3f}%'.format(tp,fn,fp,tn))
        accuracy = 100*(tp+tn)/(tp+tn+fp+fn)
        recall = 100*tp/(tp+fn)
        precision = 100*tp/(tp+fp)
        F1_Score = 2*precision*recall/(precision+recall)
        print('Accuracy={:.3f}%\tRecall={:.3f}%\tprecision={:.3f}%\tF1-score={:3f}'.format(accuracy,recall,precision,F1_Score))
        ...
        for i, j in np.ndindex(cm_percent.shape):
            ax[1].text(j, i, format(cm_percent[i, j], '.3f') + '%',
                     horizontalalignment="center",
                     color="white" if cm_percent[i, j] > thresh else "black")
        # table
        cell_text = [['{:.3f}%'.format(accuracy)],['{:.3f}%'.format(recall)],['{:.3f}%'.format(precision)],['{:.3f}%'.format(F1_Score)]]
    ...
    def plot_two(self):
        ...
        plt.plot(fpr['gene'], tpr['gene'], color='red', lw=2, label=' Gene ROC curve (area = %0.3f)' % roc_auc['gene'])
        plt.plot([0, 1], [0, 1], color='black', lw=2, linestyle='--')
        plt.xlim([-0.05, 1.05])
        plt.ylim([-0.05, 1.05])
        plt.xlabel('False Positive Rate')
